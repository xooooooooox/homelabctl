...
HOMELABCTL_INSTALL_MODE="${HOMELABCTL_INSTALL_MODE:-auto}"
...
resolve_ref() {
  local manual_ref="${HOMELABCTL_REF:-}"
  local manual_version="${HOMELABCTL_VERSION:-}"
  ...
  if [[ -z "${json}" ]]; then
    die "Failed to fetch latest release; set HOMELABCTL_VERSION or HOMELABCTL_REF."
  fi
  ...
  tag="${tag%%$'\n'*}"
  if [[ -z "${tag}" ]]; then
    die "Failed to parse latest tag; set HOMELABCTL_VERSION or HOMELABCTL_REF."
  fi
  echo "${tag}"
}
...
install_manual() {
  ...
  local install_dir="${HOMELABCTL_INSTALL_DIR:-$HOME/.local/lib/${REPO_NAME}}"
  local bin_dir="${HOMELABCTL_BIN_DIR:-$HOME/.local/bin}"
  local ref
  ref="$(resolve_ref)"
...
main() {
  local mode="${HOMELABCTL_INSTALL_MODE}"
  local pkm=""

  # Determine installation method
  case "${mode}" in
    manual)
      log "Using manual installation (HOMELABCTL_INSTALL_MODE=manual)"
      install_manual
      return 0
      ;;
