...
_homelabctl_setup_packages() {
    ...
    if [[ -f "$registry_file" ]]; then
        packages+=(${(f)"$(grep -E '^  [a-zA-Z0-9_-]+:$' "$registry_file" 2>/dev/null | sed 's/^  //' | sed 's/:$//')"})
    fi

    # Parse user registry
    if [[ -f "$user_registry" ]]; then
        packages+=(${(f)"$(grep -E '^  [a-zA-Z0-9_-]+:$' "$user_registry" 2>/dev/null | sed 's/^  //' | sed 's/:$//')"})
    fi
...
_homelabctl_setup_profiles() {
    ...
    if [[ -d "$builtin_dir" ]]; then
        for f in "$builtin_dir"/*.yaml(N); do
            profiles+=("${f:t:r}")
        done
    fi
...
_homelabctl_setup_categories() {
    ...
    if [[ -f "$registry_file" ]]; then
        categories+=(${(f)"$(sed -n '/^categories:/,/^[^ ]/p' "$registry_file" 2>/dev/null | grep -E '^  [a-zA-Z0-9_-]+:$' | sed 's/^  //' | sed 's/:$//')"})
    fi
                ...
                setup)
                    ...
                    case "${words[2]}" in
                        info)
                            _arguments \
                                '(-h --help)'{-h,--help}'[Show help]' \
                                '1:package:_homelabctl_setup_packages'
                            ;;
                        install)
                            _arguments \
                                '(-h --help)'{-h,--help}'[Show help]' \
                        ...
                        list)
                            _arguments \
                                '(-h --help)'{-h,--help}'[Show help]' \
                                '(-c --category)'{-c,--category}'[Filter by category]:category:_homelabctl_setup_categories' \
                            ...
                            case "${words[3]}" in
                                apply)
                                    _arguments \
                                        ...
                                        '1:profile:_homelabctl_setup_profiles'
                                    ;;
                                list)
                                    _arguments \
                                        '(-h --help)'{-h,--help}'[Show help]'
