...
cmd_init_all() {
  ...
  radp_log_raw "Initializing all user configurations..."
  radp_log_raw ""
  ...
  for module in setup k8s; do
    radp_log_raw "=== ${module^} Configuration ==="
  ...
  radp_log_raw "All user configurations initialized successfully"
  radp_log_raw ""
  radp_log_raw "Configuration directories:"
  radp_log_raw "  Setup: $(_setup_get_user_dir)"
  radp_log_raw "  K8s:   $(_k8s_get_extra_config_path)"
  [[ -n "$vf_config_dir" ]] && radp_log_raw "  VF:    $vf_config_dir"
  return 0
}
