...
cmd_init_k8s() {
  ...
  echo "Initializing k8s user configuration..."
  ...
  if [[ ! -f "$user_dir/addon/registry.yaml" ]] || [[ "$force" == "true" ]]; then
    ...
    echo "  Skipping addon/registry.yaml (already exists, use --force to overwrite)"
  fi
  ...
  if [[ ! -f "$user_dir/$k8s_version/README.md" ]] || [[ "$force" == "true" ]]; then
    ...
    echo "  Skipping $k8s_version/README.md (already exists, use --force to overwrite)"
  fi

  # Create .gitkeep files
  radp_exec "Create addon/profiles .gitkeep" touch "$user_dir/addon/profiles/.gitkeep"

  echo "K8s user configuration initialized at: $user_dir"
  return 0
}
...
_init_create_k8s_addon_registry() {
  ...
  echo "  Creating addon/registry.yaml"
...
_init_create_k8s_version_readme() {
  ...
  echo "  Creating $k8s_version/README.md"
